...
/*   Updated: 2024/01/22 11:39:35 by nolahmar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_h_intersection(t_vars *vars)
{
	...
	if (ray->is_down)
	{
		...
		ray->ya = -TILE_SIZE;
	ray->h_x_intersection = ((ray->h_y_intersection - vars->player_y) \
		/ tan(ray->angle));
...
void	horizontal_ray_cast(t_vars *vars)
{
	...
	while (!hit_wall)
	{
		...
		if (hit_wall == 1)
			ray->h_distance = sqrt
				(pow(ray->h_x_intersection - vars->player_x, 2.0)
					+ pow(ray->h_y_intersection - vars->player_y, 2.0));
		else
		{
...
void	find_v_intersection(t_vars *vars)
{
	...
	if (ray->is_right)
	{
		...
		ray->xa = -TILE_SIZE;
	ray->v_y_intersection = ((ray->v_x_intersection - vars->player_x) \
		* tan(ray->angle));
...
void	vertical_ray_cast(t_vars *vars)
{
	...
	while (!hit_wall)
	{
		...
		if (hit_wall == 1)
			ray->v_distance = sqrt
				(pow(ray->v_x_intersection - vars->player_x, 2.0)
					+ pow(ray->v_y_intersection - vars->player_y, 2.0));
		else
		{
